...
configurations_A <- expand.grid(imputation_method = c("smooth"),
                                grad_step_size = c(0.25, 1),
                                grad_step_rate = c(0.6, 0.8),
                                grad_iterations = c(1500, 3000),
                                samp_size = c(10^4),
                                tyk_exp = c(1),
...
configurations_B <- expand.grid(imputation_method = c("smooth"),
                                grad_step_size = c(0.25),
                                grad_step_rate = c(0.8),
                                samp_size = c(10^4, 10^4 * 5),
                                grad_iterations = c(1500, 3000),
                                tyk_exp = c(1),
                                threshold = c(1.65, 1.95),
                                min_size = 3,
                                ...
                                seed = 1:100,
                                dims = c("1D", "2D"))
configurations <- rbind(configurations_A, configurations_B) %>% as.data.table()
configurations$dims <- factor(configurations$dims, levels = c("1D", "2D"))
configurations <- configurations[order(dims, seed, threshold, mu_sd, bandwidth_sd, rho,
                                       grad_step_size, grad_step_rate, grad_iterations, samp_size)]
registerDoParallel(4)
sim_path <- "simulations/004"
