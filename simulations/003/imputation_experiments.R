...
configurations <- expand.grid(imputation_method = c("neighbors", "smooth", "none"),
                              tyk_exp = c(0.5, 1),
                              seed = 1:100,
...
nothing <- foreach(i = 1:length(filenames)) %dopar% {
  error_file <- sprintf("simulations/003/errors/iter_%s_%s_failed.txt", i, "A")
  sink(error_file)
  result <- NULL
  try(result <- run_sim(configurations[i, ]))
  if(!is.null(result)) {
    file.remove(error_file)
    filename <- sprintf("simulations/003/results/iter_%s_%s.rds", i, "A")
